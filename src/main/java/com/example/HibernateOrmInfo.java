...
public class HibernateOrmInfo {
    private final Map<String, HibernateOrmInfo.PersistenceUnit> persistenceUnits = Collections
            .synchronizedMap(new TreeMap<>(new PersistenceUnitUtil.PersistenceUnitNameComparator()));

    public Collection<HibernateOrmInfo.PersistenceUnit> getPersistenceUnits() {
        return persistenceUnits.values();
    }

    void add(HibernateOrmInfo.PersistenceUnit persistenceUnitInfo) {
        persistenceUnits.put(persistenceUnitInfo.getName(), persistenceUnitInfo);
    }

    public int getNumberOfNamedQueries() {
        return persistenceUnits.values().stream().map(HibernateOrmInfo.PersistenceUnit::getAllNamedQueries)
                .mapToInt(List::size).reduce(Integer::sum).orElse(0);
    }

    public int getNumberOfEntities() {
        return persistenceUnits.values().stream().map(HibernateOrmInfo.PersistenceUnit::getManagedEntities)
                .mapToInt(List::size).reduce(Integer::sum).orElse(0);
    }
    ...
    public static class PersistenceUnit {
        ...
        private final List<HibernateOrmInfo.Entity> managedEntities;
        private final List<HibernateOrmInfo.Query> namedQueries;
        private final List<HibernateOrmInfo.Query> namedNativeQueries;
        ...
        public PersistenceUnit(String name, List<HibernateOrmInfo.Entity> managedEntities,
                               List<HibernateOrmInfo.Query> namedQueries,
                               List<HibernateOrmInfo.Query> namedNativeQueries, Supplier<String> createDDL, Supplier<String> dropDDL,
        ...
        public List<HibernateOrmInfo.Entity> getManagedEntities() {
            return managedEntities;
        }

        public List<HibernateOrmInfo.Query> getNamedQueries() {
            return namedQueries;
        }

        public List<HibernateOrmInfo.Query> getNamedNativeQueries() {
            return namedNativeQueries;
        }

        public List<HibernateOrmInfo.Query> getAllNamedQueries() {
            ArrayList<HibernateOrmInfo.Query> allQueries = new ArrayList<>();
